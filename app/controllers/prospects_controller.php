	...
	*/
	function prospectemailtemplate(){
		...
		if(!empty($this->data))
		{
			...
			$varsearch='%'.$searchkey.'%';
			$condition = "EmailTemplate.project_id IN (0,$project_id) AND EmailTemplate.email_template_name like '$varsearch' AND EmailTemplate.delete_status='0'   AND EmailTemplate.is_sytem in('0','1') AND EmailTemplate.is_event_temp='0' AND EmailTemplate.email_template_type ='2'" ;
			//echo $condition;
		}else {
			$condition = "EmailTemplate.project_id IN(0,$project_id) AND EmailTemplate.delete_status='0'  AND EmailTemplate.is_sytem IN('0','1') AND EmailTemplate.is_event_temp='0' AND EmailTemplate.email_template_type ='2'" ;
		}

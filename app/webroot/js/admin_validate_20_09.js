...
function validatecompanytype(part){
	...
	if ($('#company_type_category_name').val() =='' && $('#hidctc').val()=='company_type_category_name' ) {
			inlineMsg('company_type_category_name','<strong>Please Select Companty Type.</strong>',2);
			return false;
	}
		
	if ($('#company_type_category_name_text').val()=='' && $('#hidctc').val()=='company_type_category_name_text' ) {
			inlineMsg('company_type_category_name_text','<strong>Please Provide Company Type.</strong>',2);
			return false;
	}
	
	if ($('#company_type_status_name').val() =='' && $('#hidcts').val()=='company_type_status_name' ) {
		inlineMsg('company_type_category_name','<strong>Please Select Company Type Status.</strong>',2);
		return false;
	}
		
	if ($('#company_type_status_name_text').val()=='' && $('#hidcts').val()=='company_type_status_name_text' ) {
			inlineMsg('company_type_status_name_text','<strong>Please Provide Company Type Status.</strong>',2);
			return false;
	}
...
function validateproject(part){
	...
	if(part == "add"){
		...
		var attr = $("select#contacts").attr('title');									

		if (typeof attr !== 'undefined' && attr !== false) {

			// Now we are in Edit case of project

			var paVal = $("select#contacts").val();

			$("#pa_contacts").val(paVal);
		...
		if($('#project_name').val() == '')
		 ...
		 if($('#companies_bb').val() != '' && $('#companies_bb').val() != 0 && $('#companies_bb').val() != null) {
			...
			var ownerval = new String($('#companies_bb').val());
			...
			if(temp.length > 1){
				...
				inlineMsg('companies_bb','<strong>Please select only one project owner.</strong>',2);				
...

		 {
			 ...
			 inlineMsg('contacts','<strong>Please select Project Administrator.</strong>',2);
		...
		if($('#address1').val() == '')

		 {
			 ...
			 inlineMsg('address1','<strong>Please provied address1</strong>',2);
	...
	if($('#zipcode').val().length < 5)
...

	 {
		 ...
		 inlineMsg('city','<strong>Please provied city</strong>',2);
	...
	if($('#old_password').val() != ''){
		...
		{	
		 ...
		 inlineMsg('new_password','<strong>Please provied new password</strong>',2);
		...
		{	
		 ...
		 inlineMsg('con_password','<strong>Please provied confirm password</strong>',2);
...
function validateplayer(actionfor, option){
		...
		if($('#company_type_id').val() == ''){
		 ...
		 if($('#category_id').val() == '' && option == 'merchant'){
			 inlineMsg('category_id','<strong>Please select Category.</strong>',2);
			 return false; 
		 }

		 if($('#ein').val() !='' && (option == 'merchant' || option == 'nonprofit')  ){
				var myRegexp = new RegExp('([0-9]{2}[-]{1}[0-9]{7})');	 
				if(!myRegexp.test($('#ein').val())){
					 inlineMsg('ein','<strong>Please provide correct format.</strong>',2);
					 return false; 
				}
		}
		
		if($('#company_type_id').val()=='67'){
			 if($('#non_profit_type_id').val() == ''){
				 inlineMsg('non_profit_type_id','<strong>Please provide Non-profit Type.</strong>',2);
				 return false; 
			 }
		}

		/*if(!$('#CompanyLocationTypeId0').is(':checked') && !$('#CompanyLocationTypeId1').is(':checked')  ){
			 inlineMsg('location_type_id','<strong>Please select Location Type.</strong>',2);
			 return false; 
		}*/
		...
		if($('#email').val() == '')
		 {
			 inlineMsg('email','<strong>Please provide Notify Email.</strong>',2);
			 return false; 
		 }
		...
		if($('#phone').val() == ''){
			 inlineMsg('phone','<strong>Please Enter Phone.</strong>',2);
			 return false; 
		 }
...
function validatecompany(actionfor){
		...
		if($('#company_type_id').val() == '')
...

		 {
		  ...
		  if($('#company_type_id').val()=='66'){

			 if($('#ein').val() == '')

			 {

				 inlineMsg('ein','<strong>Please provide EIN.</strong>',2);
...
function validateaddventorcompany(actionfor){
...
function sendToProjectDashboard(projectid,redirectUrl){
	...
	pageRedirectUrl = baseUrlAdmin + "projectdashboard";

	else

	pageRedirectUrl = baseUrlAdmin + redirectUrl;
	...
	$.ajax({
	  ...
	  success: function(data) {
		...
		window.location = pageRedirectUrl;
 ...
 * Function name   : validatenonprofittype()

 * Description     : This function is used for validation of nonprofittype type (add/edit)
...
function validatecategories($flag){
	
	if ($('#category_id').val()=='' && $('#hidcatname').val()=='category_id' ) {
		inlineMsg('category_id','<strong>Please Select Category Name.</strong>',2);
		return false;
	}
	
	if ($('#category_name_text').val()=='' && $('#hidcatname').val()=='category_name_text' ) {
		inlineMsg('category_name_text','<strong>Please Provide Category Name.</strong>',2);
		return false;
	}
	
	if ($('#sub_category_id').val()=='' && $('#hidsubcatname').val()=='sub_category_id' ) {
		inlineMsg('sub_category_id','<strong>Please Select Sub Category Name.</strong>',2);
		return false;
	}
	
	if ($('#sub_category_name_text').val()=='' && $('#hidsubcatname').val()=='sub_category_name_text' ) {
		inlineMsg('sub_category_name_text','<strong>Please Provide Sub Category Name.</strong>',2);
		return false;
	}
...
function validateoffertypes($flag){

	

	if ( $('#offer_type_template_id').val() == '') {

		inlineMsg('offer_type_template_id','<strong>Please Select Offer Type.</strong>',2);
	...
	if ($('#offer_type_name').val()=='' ) {
		...
		inlineMsg('offer_type_name','<strong>Please Provide Offer Type Name.</strong>',2);
...
function validatenonprofittype($flag){

	if ($('#non_profit_type_name').val()=='' ) {

		inlineMsg('non_profit_type_name','<strong>Please Provide Non-Profit Type.</strong>',2);

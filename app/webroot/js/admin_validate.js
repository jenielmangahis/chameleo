...
function validatecompanytype(part){
   ...
   /*  if($('#typename').val() == '')
	...
	if ($('#company_type_status_name').val() =='' && $('#hidcts').val()=='company_type_status_name' ) {
		inlineMsg('company_type_status_name','<strong>Please Select Company Type Status.</strong>',2);
		return false;
	}
...
function validatemailcontent(actionfor){
	...
	if($('#email_template_type').val() == '')
	 {
		 inlineMsg('email_template_type','<strong>Please select Relationship Type.</strong>',2);
		 return false; 
	 }
...
function validate_responder(actionfor){
    ...
    if(validateemail($('#sender').val()) == false &&  $('#sender').val() != 'info@[[Project Website Address]]' ){
             inlineMsg('sender',"<strong>Please enter valid sender email.</strong>",2);
             return false; 
    }
...
function validatecategories($flag){
	...
	if ($('#category_id').val()=='' && $('#hidcatname').val()=='category_id' ) {
		inlineMsg('category_id','<strong>Please Provide Category Name.</strong>',2);
		return false;
	}
	...
	if ($('#sub_category_id').val()=='' && $('#hidsubcatname').val()=='sub_category_id' ) {
		inlineMsg('sub_category_id','<strong>Please Provide Sub Category Name.</strong>',2);
		return false;
	}

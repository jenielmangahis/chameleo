...
class MemberType extends AppModel{
 ...
 function getMemberTypesByProjectCondition($project_id, $searchkey=""){
     if($project_id){
            $condition=" MemberType.project_id='".$project_id."' and MemberType.delete_status='0' ";          
            if($searchkey!=""){
                $condition .= " AND ( MemberType.member_type like '%".$searchkey."%' ) ";             
            }
             return $condition;  
     }else{
         return "  MemberType.delete_status='0' ";          
     }

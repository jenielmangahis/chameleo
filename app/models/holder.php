<?php
    ...
    */
    ...
    class Holder extends AppModel{
        ...
        function isUniqueScreenName($screen_name, $project_id, $holder_id = null)
        {

            //echo 	$screen_name.$project_id.$holder_id."<br/>";
            if ($holder_id == null) {
                $conditions = array("Holder.screenname = '$screen_name'" , "Holder.project_id = $project_id");
            }
            else {
                $conditions = array("Holder.screenname = '$screen_name'" , "Holder.id != $holder_id", "Holder.project_id = $project_id");
            }
        ...
        * @param mixed $page
        */
        function getMemberListByProject($project_id, $member_type="", $searchkey="", $order="", $limit="", $page=""){
            $resultArray=null;        
            if($project_id){
                $holderCond=" Holder.project_id = '".$project_id."' AND Holder.delete_status='0' ";
				
				App::import("Model", "MemberType");  
           		$this->MemberType =  & new MemberType();
				if($member_type != MemberType::MEMBER_TYPE_NON_MEMBER ){
					$holderCond .= " AND Holder.member_type != 52"; 
				}
                ...
                $fields=" Holder.*, MemberType.*, DonationLevel.*, (select sum(points) from point_archive_users where member_id=Holder.user_id) as totalpoints";

		...
		<tr>
			<td colspan='3'><?php if($session->check('Message.flash')){ $session->flash(); }
                ...
                <tr>
                    ...
                    <td><span class="intpSpan"><label for="title"></label>   <?php echo $form->input("Project.project_name", array('id' => 'project_name', 'div' => false, 'label' => '',"class" => "inpt_txt_fld","maxlength" => "200",'onchange' => 'ajaxuniqueprojectname(this.value)'));?>
                    </span>
                     </td>
				</tr>
                <tr>
                    <td align="right"> <label class="boldlabel">System Project Name <span class="red">*</span></label></td>
                    <td><span class="intpSpan"><label for="title"></label>   <?php echo $form->input("Project.system_name", array('id' => 'system_name', 'div' => false, 'label' => '',"class" => "inpt_txt_fld","maxlength" => "200",'onchange' => 'ajaxuniquesystemname(this.value)'));?>
                        </span>
                    </td>
                </tr>
                ...
                <tr>
					...
					<?php  echo $form->radio("Project.relation_type", array('Direct'=>'Direct','3rd Party'=>'3rd Party'), array('default'=>'Direct','id'=>'relation_type', 'legend'=>false,'style'=>'','class'=>'change_rel_type')); ?>     
                    </td>
                </tr> 
			...
			<tr id="distributor_content">
                ...
                <tr>
                    ...
                    <td valign="top">
                            ...
                            <div id="contactemails" style=" background: none repeat scroll 0 0 #EBEBEB;  border: 1px solid #D3D3D3; display: block; font-size: 13px; height: 112px; overflow: auto; width: 100%;" > 
                            <table width="100%">
...
<tr>
                    ...
                    <td><?php echo $totalnumunits; ?></td>
				</tr>
<?php } ?>

<?php if(isset($this->data['Project']['id'])){ ?>
<tr style="height: 22px;">
                        <td  align="right" style="padding-bottom: 10px; width: 165px;" ><label class="boldlabel"># of Members</label></td>
...
<?php } ?>
	        ...
	        <tr><td>&nbsp;</td><td colspan='2'><div id="loadingdivimg" style="display:none">Loading..<img src="/img/ajax-loader.gif"></td></tr>
...
<?php } ?>
             ...
             <td>
				...
				<span class="btnLft">				
					<input type="button" value="Add"   class="btnRht" name="Add"  ONCLICK="javascript:(window.open('<?php echo $ProjectOwnerAddUrl ?>'))" />
			</span>
			<span style="margin-left:8px">				
				<?php echo $html->image('/img/refresh_btn.png', array('height' =>'20px','widht' =>'20px','title' => 'Reloade project owner','onclick' => 'loadcompany()'));?>
			</span>
             ...
             <td>
							<div >
                                <span class="txtArea_top">
									...
									<?php
									 //echo $form->select('ProjectOwner.contacts',$contects, null,array('id'=>'contacts','size'=>'1','empty'=>false,'class'=>'multilist multi','empty'=>false,'default'=>$contactsIds));
									 echo $form->select('Project.project_contact_admin_id',$contects, null,array('id'=>'contacts','size'=>'1','empty'=>false,'class'=>'multilist multi','empty'=>false,'default'=>$contactsIds));
									?>
									</span>
								</span>
                            </div>
			 ...
			 <!--
			 <tr>
             <td align="right"><label class="boldlabel">System Owner<span class="red">*</span></label></td>
             <td><span class="intpSpan">
			 </span></td>
             </tr>
			 -->
			 <?php echo $form->hidden("Sponsor.sponsor_name", array('id' => 'sponsor_name', 'div' => false, 'label' => '',"class" => "inpt_txt_fld","maxlength" => "150"));?>
             <tr>
             <td align="right"><label class="boldlabel">
			 <?php echo ($this->data['Project']['id'])?'':'Temp';?>
			  Username <span class="red">*</span></label></td>
             <td><span class="intpSpan"><?php 
			 //echo $this->data['User']['username'];
			 if(isset($this->data['User']['username']) && !empty($this->data['User']['username'])){
				echo $form->input("Sponsor.username", array('id' => 'username', 'div' => false, 'label' => '',"class" => "inpt_txt_fld","maxlength" => "150",'disabled'=>'disabled','onchange' => 'ajaxuniquesponsorname(this.value)','value'=>$this->data['User']['username']));		
			 } else {
				echo $form->input("Sponsor.username", array('id' => 'username', 'div' => false, 'label' => '',"class" => "inpt_txt_fld","maxlength" => "150",'onchange' => 'ajaxuniquesponsorname(this.value)'));
			 }
...
<script>
	...
	$('#system_name').bind('keyup keypress blur', function() {  
	...
	function loadCompanyNames(str,str2) {
		//alert(str);
...
function get_all_information(rel_type){
    ...
    function get_sys_pricing_info(sys_pri_id,check)
    {
        ...
        $.ajax({
                    ...
                    dataType:'json',
                    success: function(output){
                        //alert(output.super_footer);
						$("#system_monthly_charge").val('');
						$("#is_shoppingcartenabled").attr("checked",false);
						$("#is_superfooterenabled").attr("checked",false);
						$('#border_footer_id').attr("disabled", true);						
						
						if(output != null && output != "" ) {
							if(output.monthly_charge!="")
							...
							if(output.shopping_cart!="")
							{
								...
								if(shopping_cart==1)
								{
									$("#is_shoppingcartenabled").attr("checked",true);
								}
							}
							if(output.super_footer==1)
							{
							$("#is_superfooterenabled").attr("checked",true); 
							$('#border_footer_id').attr("disabled", false);
							}
                        } 
                    }
...
<script>
	...
	loadCompanyNames(alreadyCid,alreadyConStr);
</script>
<script>
	function loadcompany(){
		$.ajax({
			type:"POST",
			url: baseUrlAdmin+"getallcompanydata",
			//cache:false,
			success: function(output){
				$('select#companies_bb').html(output);				
			}
		});
	}
</script>

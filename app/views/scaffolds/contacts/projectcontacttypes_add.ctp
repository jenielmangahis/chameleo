...
$backUrl = $base_url.'contacts/projectcontacttypes';

?>

<div class="titlCont1"><div class="centerPage">
<?php echo $form->create("contacts", array("action" => "projectcontacttypes_add",'name' => 'projectcontacttypes_add', 'id' => "projectcontacttypes_add",'onsubmit' => 'return validatecontacttype("add");'));
...
?>
  ...
  <span class="titlTxt"><?php echo $pageactionname;?> Contact Type </span>
		...
		<li><button type="submit" value="Submit" class="button" name="data[Action][noredirection]"><span>Apply</span></button></li>
             <li><button type="button" id="saveForm" class="button"  <?php if($returnurl){ echo "onclick='closemywindow();'"; }else{?>ONCLICK="javascript:(window.location='<?php echo $backUrl;?>')"  <?php } ?>><span> Cancel</span></button></li>
                </ul>
        </div>
</div></div>	
<div class="midPadd">

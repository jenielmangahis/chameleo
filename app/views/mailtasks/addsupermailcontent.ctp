...
<li><button type="button" id="saveForm" class="button"  <?php if($returnurl){ echo "onclick='closemywindow();'"; }else{?>ONCLICK="Backurlsupermailcontent()" <?php } ?>><span> Cancel</span></button></li>
</ul>
</div>
</div></div>
	...
	<div class="boxBor1">
		...
		<?php echo $form->select("EmailTemplate.email_template_type",$templatetypedropdown,$selectedtemplatetype, array('id' => 'email_template_type', 'div' => false, 'label' => '',"class" =>"multilist","maxlength" => "250"),"---Select---"); ?>
				...
				<td colspan=2>
                        ...
                        ?>
             <script type="text/javascript">
				//var ck_newsContent = CKEDITOR.replace('content',{filebrowserBrowseUrl : baseUrl+'js/ckfinder/ckfinder.html',filebrowserWindowWidth : '800',filebrowserWindowHeight : '700'});
			 </script>
                        </td>
                    </tr>       
...
<script type="text/javascript">
	...
	function Backurlsupermailcontent(){
		window.location=baseUrl+'mailtasks/supermailtemplatelist';
	}
</script>
